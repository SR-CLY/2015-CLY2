...
RIGHT_COMPENSATION = 0.977 # Multiplier used to counter the effects of the right motor being more powerful 
FULL_TURN = 1.9  # Seconds
...
def turn_old(robot, angle, direction, power=50):
    ...
    duration = abs((angle*4) * ((FULL_TURN/2)/360))
    if direction == RIGHT:
        print("RIGHT")
        set_motor_power(robot, power, 0)
        set_motor_power(robot, -power, 1)
...
def half_drive_to(robot, log, marker, power=100):
    """Drives halfway to a specified marker"""
